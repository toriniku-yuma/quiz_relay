...
- JUDGED：正解なら次問。誤答ならお手つきを1回加算し当該問題をロック、累計3回で失格として以後のbuzz・文字選択を拒否する。失格していない残りの参加者へ凍結位置から再開。全員ロック済みなら次問。ただし7問先取ならFINISHEDへ進み、失格していない参加者が1人以下ならINVALIDへ進む。問題枯渇時はその時点の成績で判定してFINISHEDへ進む（比較・同数時の処理は仕様の仮案）。
